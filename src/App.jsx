...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f8f7ff] text-gray-800 flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8 relative overflow-x-hidden">
      ...
      {statusMessage && (
        <div className={`fixed top-6 z-[120] px-6 py-4 rounded-2xl shadow-2xl font-bold flex items-center justify-center gap-3 animate-bounce ${statusMessage.type === 'success' ? 'bg-purple-500 text-white' : 'bg-red-500 text-white'}`}>
      ...
      <div className="w-full max-w-lg md:max-w-xl lg:max-w-2xl bg-white rounded-[2.5rem] sm:rounded-[3.5rem] shadow-[0_40px_100px_rgba(147,51,234,0.1)] overflow-hidden border border-white relative mb-8 transition-all duration-500">
        ...
        {step > 0 && (
          <div className="px-6 sm:px-10 pt-8 text-left">
            ...
            <div className="flex justify-between mt-4 items-center">
              <button onClick={back} className="text-gray-300 hover:text-purple-400 flex items-center justify-center gap-1 font-black text-[10px] sm:text-xs uppercase tracking-widest transition-colors">
        ...
        {step === 1 && (
          <div className="p-8 sm:p-10 space-y-8 text-left animate-in">
            ...
            <button onClick={next} disabled={!formData.parentName || formData.phone.length < 8} className="w-full py-6 bg-purple-500 text-white rounded-3xl font-black text-lg shadow-xl shadow-purple-100 disabled:opacity-30 active:scale-[0.98] transition-all flex items-center justify-center">Siguiente</button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="p-8 sm:p-10 space-y-8 text-left animate-in">
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-4 pt-4 border-t border-gray-100">
                <div className="flex items-center gap-2 mb-2">
                   ...
                   <p className="text-[10px] font-bold text-gray-400 leading-tight">Nota: Si no incluyes el colegio o grado, esta información no aparecerá en tus etiquetas.</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 block">Colegio (Opcional)</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 block">Grado (Opcional)</label>
            ...
            <button onClick={next} disabled={!formData.studentName || !formData.theme} className="w-full py-6 bg-purple-500 text-white rounded-3xl font-black text-lg shadow-xl shadow-purple-100 disabled:opacity-30 active:scale-[0.98] transition-all flex items-center justify-center">Elegir Productos</button>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="p-6 sm:p-10 space-y-6 text-left animate-in">
            ...
            <button onClick={next} disabled={!formData.selectedKit || (formData.selectedKit === 'ninguno' && formData.selectedItems.length === 0)} className="w-full py-6 bg-purple-500 text-white rounded-3xl font-black text-lg shadow-xl shadow-purple-100 disabled:opacity-30 active:scale-[0.98] transition-all flex items-center justify-center">Siguiente</button>
          </div>
        )}
          ...
          <div className="p-8 sm:p-10 space-y-8 text-left animate-in">
            ...
            <div className="grid grid-cols-1 gap-4">
              <button onClick={() => setFormData({...formData, paymentMethod: 'transferencia'})} className={`p-6 border-2 rounded-[2rem] flex items-center gap-4 transition-all ${formData.paymentMethod === 'transferencia' ? 'border-purple-500 bg-purple-50 shadow-md' : 'border-gray-50 bg-white hover:border-purple-100'}`}>
                ...
                <div className="text-left"><p className="font-black text-gray-700">Transferencia</p><p className="text-[10px] text-gray-400 font-bold">Coordina los datos tras la aprobación</p></div>
              </button>
              <button onClick={() => setFormData({...formData, paymentMethod: 'mercado_pago'})} className={`p-6 border-2 rounded-[2rem] flex items-center gap-4 transition-all ${formData.paymentMethod === 'mercado_pago' ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-50 bg-white hover:border-blue-100'}`}>
                ...
                <div className="text-left"><p className="font-black text-gray-700">Mercado Pago</p><p className="text-[10px] text-gray-400 font-bold">Link de pago tras la aprobación</p></div>
              </button>
            </div>
            <button onClick={next} disabled={!formData.paymentMethod} className="w-full py-6 bg-purple-500 text-white rounded-3xl font-black text-lg shadow-xl shadow-purple-100 disabled:opacity-30 active:scale-[0.98] transition-all flex items-center justify-center">Siguiente</button>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="p-6 sm:p-10 space-y-6 text-left animate-in">
            ...
            <div className="flex flex-row gap-4">
              <button onClick={() => setFormData({...formData, deliveryType: 'retiro', commune: '', street: '', houseNumber: '', receiverName: formData.parentName})} className={`flex-1 p-6 border-2 rounded-[2rem] flex flex-col items-center justify-center gap-2 font-black transition-all ${formData.deliveryType === 'retiro' ? 'border-purple-500 bg-purple-50 text-purple-600 shadow-md' : 'border-gray-50 text-gray-300 hover:border-purple-100'}`}>
                <MapPin size={24} /> <span className="text-sm">Retiro</span>
              </button>
              <button onClick={() => setFormData({...formData, deliveryType: 'envio'})} className={`flex-1 p-6 border-2 rounded-[2rem] flex flex-col items-center justify-center gap-2 font-black transition-all ${formData.deliveryType === 'envio' ? 'border-purple-500 bg-purple-50 text-purple-600 shadow-md' : 'border-gray-50 text-gray-300 hover:border-purple-100'}`}>
            ...
            {formData.deliveryType === 'retiro' && (
              <div className="p-6 bg-blue-50 rounded-[2rem] border border-blue-100 flex gap-4 animate-in">
                ...
                <p className="text-sm font-bold text-blue-700 leading-relaxed">Podrás retirar tu pedido en el punto de entrega que te indicaremos de manera personalizada a través de WhatsApp una vez que el pedido esté listo.</p>
              </div>
            )}
            ...
            <button onClick={next} disabled={!formData.deliveryType || !formData.deliveryDateId || (formData.deliveryType === 'envio' && (!formData.commune || !formData.street || !formData.houseNumber))} className="w-full py-6 bg-purple-500 text-white rounded-3xl font-black text-lg shadow-xl shadow-purple-100 disabled:opacity-30 active:scale-[0.98] transition-all flex items-center justify-center">Ver Resumen</button>
          </div>
        )}

        {step === 6 && (
          <div className="p-8 sm:p-10 text-center animate-in">
            ...
            <div className="p-6 bg-orange-50 border border-orange-100 rounded-3xl mb-8 text-left">
              <p className="text-xs font-bold text-orange-800 leading-relaxed italic">* Importante: Para poder aceptar el pedido debe pagarse el 50% del total mediante el medio de pago señalado. Una vez enviada la solicitud, recibiremos la notificación, validaremos stock y te enviaremos el link o datos de pago por WhatsApp para confirmar.</p>
            </div>
            <div className="bg-gray-50 p-6 sm:p-8 rounded-[2rem] text-left space-y-4 border border-gray-100 mb-8 relative overflow-hidden shadow-sm">
              ...
              <div className="relative z-10 space-y-4">
                <div className="text-left"><p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Datos de Contacto</p><p className="font-black text-gray-700 text-lg leading-tight">{formData.parentName} <span className="text-gray-300 mx-1">|</span> +56 {formData.phone}</p></div>
                ...
                <div className="pt-4 border-t border-gray-200/50">
                  ...
                  <div className="space-y-2">
                    <p className="font-black text-purple-600 text-sm flex justify-between"><span>• {KITS.find(k => k.id === formData.selectedKit)?.name}</span><span>${KITS.find(k => k.id === formData.selectedKit)?.price.toLocaleString()}</span></p>
                    {formData.selectedItems.map(id => (<p key={id} className="font-bold text-gray-500 text-[10px] sm:text-[11px] flex justify-between ml-2"><span>+ {INDIVIDUAL_PRODUCTS.find(p => p.id === id).name}</span><span>${INDIVIDUAL_PRODUCTS.find(p => p.id === id).price.toLocaleString()}</span></p>))}
                    <p className="font-bold text-blue-600 text-xs mt-2 border-t border-blue-100 pt-2 flex justify-between"><span>💳 Pago elegido:</span><span className="uppercase">{formData.paymentMethod === 'transferencia' ? 'Transferencia' : 'Mercado Pago'}</span></p>
                  </div>
                </div>
              </div>
              <div className="pt-4 border-t border-purple-100 flex justify-between items-end"><p className="font-black text-gray-800 text-xl">Total</p><p className="font-black text-purple-500 text-3xl sm:text-4xl tracking-tighter">${totalAmount.toLocaleString()}</p></div>
            </div>
            ...
            <button onClick={() => setStep(5)} className="mt-6 text-gray-300 font-bold text-xs underline block w-full text-center hover:text-purple-400 transition-colors uppercase tracking-widest">Corregir datos</button>
          </div>
        )}
      </div>
      
      {/* Admin Toggle */}
      <button onClick={() => setShowAdmin(true)} className="mt-8 text-gray-300 hover:text-purple-400 transition-all flex items-center justify-center gap-2 font-black text-[10px] uppercase tracking-widest bg-white/50 px-8 py-3 rounded-full border border-white/50 backdrop-blur-sm">
            ...
            <div className="p-4 sm:p-10 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50/50">
              ...
              {isAdminAuth && (
                <div className="flex bg-gray-200/50 p-1 rounded-2xl w-full sm:w-auto">
                  <button onClick={() => setAdminTab('activos')} className={`flex-1 sm:flex-none px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${adminTab === 'activos' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}><ClipboardList size={14}/> Activos</button>
                  <button onClick={() => setAdminTab('historial')} className={`flex-1 sm:flex-none px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${adminTab === 'historial' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}><History size={14}/> Historial</button>
                </div>
              )}
            ...
            <div className="p-4 sm:p-10 overflow-y-auto flex-1 bg-white">
              {!isAdminAuth ? (
                <div className="py-10 flex flex-col items-center gap-6 text-center max-w-xs mx-auto">
                  ...
                  <button onClick={handleAdminLogin} className="w-full py-5 bg-purple-500 text-white rounded-3xl font-black shadow-lg shadow-purple-100 flex items-center justify-center">Acceder al Sistema</button>
                </div>
              ) : (
                <div className="space-y-10 pb-10 text-left">
                  {adminTab === 'historial' && (
                    <div className="flex flex-col sm:flex-row gap-4">
                       <div className="relative group flex-1">
                          ...
                          <input type="text" placeholder="Busca por nombre, temática, colegio, celular o kit..." className="w-full p-6 pl-16 bg-gray-50 border-none rounded-[2.5rem] font-bold text-gray-700 outline-none shadow-inner focus:ring-4 focus:ring-purple-100 transition-all" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}/>
                       </div>
                       <button onClick={() => setShowClearModal(true)} className="p-6 bg-red-50 text-red-500 rounded-[2.5rem] font-black uppercase text-xs flex items-center justify-center gap-2 hover:bg-red-100 transition-all"><Trash2 size={18}/> Limpiar Historial</button>
                    </div>
                  )}
                  ...
                  {adminTab === 'activos' && (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      <div className="bg-gradient-to-br from-purple-600 to-indigo-600 p-8 rounded-[3rem] text-white shadow-xl shadow-purple-100 flex flex-col justify-between">
                        <div className="flex justify-between items-start"><TrendingUp size={24}/><span className="text-[10px] font-black uppercase tracking-widest opacity-60">Ingresos Brutos (Finalizados)</span></div>
                      ...
                      <div className="lg:col-span-2 bg-gray-50 p-8 rounded-[3rem] border border-gray-100">
                        <div className="flex items-center gap-2 mb-6"><BarChart3 size={18} className="text-purple-500"/><h4 className="font-black text-gray-800 text-lg">Performance por Kit</h4></div>
                        <div className="space-y-4">
                  ...
                  <div className="space-y-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-2">
                       <h4 className="font-black text-gray-800 text-xl flex items-center gap-2">{adminTab === 'activos' ? <ClipboardList size={20} className="text-indigo-500"/> : <History size={20} className="text-gray-500"/>}{adminTab === 'activos' ? 'Pedidos por Aprobar/Procesar' : 'Historial Lelune'}</h4>
                       {adminTab === 'historial' && (
                    ...
                    <div className="flex flex-col gap-4">
                      {filteredOrders.length === 0 ? (<div className="py-20 text-center text-gray-300 font-black uppercase tracking-widest bg-gray-50 rounded-[2rem] border border-dashed flex flex-col items-center gap-4"><ShoppingBag size={48} className="opacity-20"/>No se encontraron pedidos</div>) : (
                          ...
                          return (
                            <div key={o.id} className={`w-full p-6 sm:p-8 rounded-[2rem] border transition-all flex flex-col gap-6 ${o.status === 'listo' ? 'bg-green-50/50 border-green-100' : o.status === 'rechazado' ? 'bg-red-50 opacity-80 border-red-100' : 'bg-white border-gray-100 shadow-sm'}`}>
                              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                                <div className="space-y-1">
                                  <div className="flex flex-wrap items-center gap-3">
                                    ...
                                    {o.status === 'rechazado' && <span className="bg-red-500 text-white text-[9px] font-black px-2 py-1 rounded-lg uppercase">Rechazado/Cancelado</span>}
                                  ...
                                  <p className="font-black text-gray-800 text-2xl leading-none">{o.parentName}</p>
                                </div>
                                <div className="text-left lg:text-right"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Total</p><p className="font-black text-purple-600 text-3xl tracking-tighter leading-none mt-1">${o.total?.toLocaleString()}</p></div>
                              </div>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 md:flex md:flex-row gap-3 w-full border-t border-gray-100 pt-6">
                                <a href={`https://wa.me/56${o.phone}`} target="_blank" className="p-4 bg-green-500 text-white rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-green-100 hover:scale-[1.02] transition-all"><Phone size={14}/> WhatsApp</a>
                                {o.status === 'pendiente' && (<><button onClick={() => updateOrderStatus(o.id, 'aceptado')} className="p-4 bg-blue-500 text-white rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-100 hover:scale-[1.02] transition-all"><ThumbsUp size={14}/> Aceptar (Link)</button><button onClick={() => updateOrderStatus(o.id, 'rechazado')} className="p-4 bg-red-500 text-white rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-red-100 hover:scale-[1.02] transition-all"><ThumbsDown size={14}/> Rechazar</button></>)}
                                {o.status === 'aceptado' && (<><button onClick={() => updateOrderStatus(o.id, 'listo')} className="p-4 bg-[#8e24aa] text-white rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-purple-100 hover:scale-[1.02] transition-all md:flex-1"><CheckCircle2 size={14}/> Confirmar Pago</button><button onClick={() => updateOrderStatus(o.id, 'rechazado')} className="p-4 bg-red-500 text-white rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-red-100 hover:scale-[1.02] transition-all"><ThumbsDown size={14}/> Cancelar Pedido</button></>)}
                                {o.status === 'rechazado' && (<button onClick={() => updateOrderStatus(o.id, 'pendiente')} className="p-4 bg-gray-100 text-gray-400 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest hover:bg-gray-200 transition-all md:flex-1">Reabrir Solicitud</button>)}
                              </div>
                            </div>
                          );
                        })
      ...
      {showClearModal && (
        <div className="fixed inset-0 bg-black/50 z-[200] flex items-center justify-center p-4 backdrop-blur-sm animate-in">
          <div className="bg-white p-8 rounded-[3rem] max-w-sm w-full shadow-2xl text-center space-y-6">
            <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center mx-auto text-red-500"><AlertCircle size={32}/></div>
            <div><h3 className="text-xl font-black text-gray-800 mb-2 text-center">¿Limpiar Historial?</h3><p className="text-sm font-bold text-gray-400 text-center">Esta acción eliminará todos los pedidos finalizados y rechazados permanentemente de la base de datos. Esta acción no se puede deshacer.</p></div>
            <div className="flex gap-3">
              <button onClick={() => setShowClearModal(false)} className="flex-1 p-4 bg-gray-100 text-gray-500 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center">Cancelar</button>
              <button onClick={clearHistory} disabled={loading} className="flex-1 p-4 bg-red-500 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg shadow-red-100 flex items-center justify-center">{loading ? 'Borrando...' : 'Sí, Limpiar'}</button>
            </div>
          </div>
        </div>
      )}
